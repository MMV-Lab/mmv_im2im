...
class Model(pl.LightningModule):
    def __init__(self, model_info_xx: Dict, train: bool = True):
        ...
        if train: 
            ...
            self.lambda_l1 = 100
            self.save_checkpoint_n_epochs= 5
    ...
    def training_step(self, batch, batch_idx,optimizer_idx):
        ...
        if self.current_epoch % (self.save_checkpoint_n_epochs)==0:
            self.trainer.save_checkpoint(f'pix2pix_model_{self.current_epoch}_state.ckpt')
        if optimizer_idx==0:
             total_accuracy=[]
             G_loss_real = self.criterion_BCE(pred_fake, torch.ones_like(pred_fake))
             L1_loss = self.criterion_L1(fake_image_B, image_B)
             ...
             output = OrderedDict({"loss": D_Loss, "progress_bar": tqdm_dict, "log": tqdm_dict,"accuracy":d_total_acu})
             return output
