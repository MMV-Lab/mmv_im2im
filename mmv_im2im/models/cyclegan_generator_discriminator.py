...
# Adapted for neoamos 3D implementation of pix2pix
# and CycleGAN and davidiommi CycleGAN
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, norm_layer, use_dropout, use_bias):
        ...
        self.conv_block = self.build_conv_block(
                            dim, norm_layer, use_dropout, use_bias
                            )
    ...
    def build_conv_block(self, dim, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [
                    nn.Conv3d(
                        dim, dim, kernel_size=3, padding=0, bias=use_bias
                        ),
                    norm_layer(dim),
                    nn.ReLU(True)
                    ]
...
class Generator(nn.Module):
    def __init__(
        ...
        for i in range(n_blocks):
            model += [
                        ResnetBlock(
                            ngf * mult, norm_layer=norm_layer,
...
class Discriminator(nn.Module):
    def __init__(
                ...
                norm_layer=nn.InstanceNorm3d, use_sigmoid=False):
