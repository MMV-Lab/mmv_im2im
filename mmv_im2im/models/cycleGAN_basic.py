...
class Model(pl.LightningModule):
    def __init__(self, model_info_xx: Dict, train: bool = True):
        ...
        self.generator_model_image_A = parse_config(model_info_xx["generator_net"])
        self.generator_model_image_B = parse_config(model_info_xx["generator_net"])
        self.discriminator_model_image_A = parse_config(model_info_xx["discriminator_net"])
        self.discriminator_model_image_B = parse_config(model_info_xx["discriminator_net"])
        ...
        if train:
            ...
            self.cycle_w = 10
            self.id_w = 2
            self.save_checkpoint_n_epochs=2
    ...
    def configure_optimizers(self):
        self.optim_gen = self.optimizer_func(list(self.generator_model_image_A.parameters())+list(self.generator_model_image_B.parameters()))
        self.optim_disc = self.optimizer_func(list(self.discriminator_model_image_A.parameters())+list(self.discriminator_model_image_B.parameters()))
    ...
    def training_step(self, batch, batch_idx,optimizer_idx):
        ...
        if self.current_epoch % (self.save_checkpoint_n_epochs)==0:
            self.trainer.save_checkpoint(f'cyclegan_model_{self.current_epoch}_state.ckpt')
        if optimizer_idx==1:  
            D_B_real_loss=self.MSE_criterion(D_B_real, torch.ones_like(D_B_real))
            D_B_fake_loss=self.MSE_criterion(D_B_fake, torch.zeros_like(D_B_fake))
            D_B_loss=D_B_real_loss+D_B_fake_loss
            D_A_real_loss=self.MSE_criterion(D_A_real, torch.ones_like(D_A_real))
            D_A_fake_loss=self.MSE_criterion(D_A_fake, torch.zeros_like(D_A_fake))
            ...
            tqdm_dict = {"g_loss": G_loss,'validity': validity_loss, 'reconstr': cycle_loss, 'identity': identity_loss}
    ...
    def validation_epoch_end(self, validation_step_outputs):
        validation_generator_loss=torch.stack([x["generator_loss"] for x in validation_step_outputs],dim=0).mean().item()
        validation_discriminator_loss=torch.stack([x["discriminator_loss"] for x in validation_step_outputs],dim=0).mean().item()
        self.log("validation_loss_generator",validation_generator_loss)
        self.log("validation_loss_discriminator",validation_discriminator_loss)
