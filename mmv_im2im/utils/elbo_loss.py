...
class ELBOLoss(nn.Module):
    ...
    def __init__(self, beta: float = 1.0, n_classes: int = 2,
                 ...
                 fractal_weight: float = 0.1, fractal_num_kernels: int = 5,
                 fractal_mode: str = "classic", fractal_to_binary: bool = True,
                 use_topological_regularization: bool = False,
                 topological_weight: float = 0.1, topological_dim: int = 2,
                 topological_connectivity: int = 4, topological_inclusion: list = None,
                 topological_exclusion: list = None, topological_min_thick: int = 1,
                 use_connectivity_regularization: bool = False,
                 connectivity_weight: float = 0.1, connectivity_kernel_size: int = 3,
        ...
        if elbo_class_weights is not None:
            self.elbo_class_weights = torch.tensor(elbo_class_weights, dtype=torch.float32)
        else:
    ...
    def forward(self, logits, y_true, prior_mu, prior_logvar, post_mu, post_logvar):
        ...
        if self.elbo_class_weights is not None and self.elbo_class_weights.device != logits.device:
        ...
        log_likelihood = -F.cross_entropy(logits, y_true_squeezed.long(), reduction='mean', weight=elbo_class_weights_on_device)
        ...
        if self.use_topological_regularization:
            ...
            if y_true_squeezed.ndim == 3:
                ...
                y_true_for_topological = y_true.float() # This should already be B, 1, H, W

            # logits are B, C, H, W (or B, C, H, W, D), which is what TI_Loss expects for x
            topological_loss = self.topological_loss_calculator(logits, y_true_for_topological)
        ...
        if self.use_connectivity_regularization:
            ...
            y_true_one_hot = F.one_hot(y_true_squeezed.long(), num_classes=self.n_classes).permute(0, 3, 1, 2).float()

            connectivity_loss = self.connectivity_coherence_calculator(y_pred_softmax, y_true_one_hot)
        ...
        if self.use_gdl_focal_regularization:
            ...
            gdl_focal_loss = self.gdl_focal_loss_calculator(logits, y_true_for_gdl_focal)
