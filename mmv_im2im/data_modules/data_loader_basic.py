...
from mmv_im2im.utils.misc import generate_dataset_dict_monai, aicsimageio_reader, parse_config_func_without_params
import random
import monai
...
class Im2ImDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        val_dataset_func = parse_config_func_without_params(val_loader_info["dataloader_type"])
        val_dataset = val_dataset_func(data=self.val_data, transform=self.preproc, **val_loader_info["dataset_params"])
        val_dataloader = DataLoader(val_dataset, shuffle=True, collate_fn=list_data_collate, **val_loader_info["dataloader_params"])
